...
Everything we need for mocking comes from vi:

```ts
import { vi } from 'vitest';

vi.fn(); // create a mock function => a fake you can control
vi.stubGlobal(); // replace a global (like fetch or window.location)
vi.spyOn(); // spy on an existing function without replacing it
vi.restoreAllMocks(); // put everything back after the test
```
